...
Might have additional useful information about these employees.

Such as the region where they work, how long they've worked there, 
and some measure of how well they're performing at a 90 day review.

USE PAINTING

So there might be questions that we want to answer that involve data
from these two sets. E.g., we were looking at the relationship between
training huors and perfrmance befre. And we had seen that employees
with more training hours had a higher performance index.

But maybe that relationship differs across people who have been with the
company for different amounts of time. For example, maybe completing more
training hours is especially helpful for employees who are just starting out
and have been with the company for less than a year.

If we wanted to look at that, we need to pull information from BOTH 
these datasets.
...
Showing just a subset of the columns using select
...
**use `cor` to calculate correlations**
...
Now this is what we want to VISUALIZE. 
We've got this correlation matrix, and we can spend a while staring at the
numbers and trying to interpret what they mean. For example...


But wouldn't it be so much easier to grasp these correlations in
a visual form? Like, what if we could use color to indicate
whether a correlation was positive or negative, and how strong
the correlation is?

So that's our goal. Take this numeric matrix and turn it 
into a more easy to grasp visual.

At this point we have a couple different options. There are various 
packages that people have written that visualize correlation matrices.
...
```{r relational-data-cor-mat-1B-small, echo=FALSE}
kable(cor_mat) %>%
  kable_styling(font_size = 18) %>%
...
::: fragment
```{r relational-data-cor-mat-1C, echo=FALSE}
cor_tidy <- cor_mat %>%
  as.data.frame() %>%
...
::: notes
So I'm going to show you one option, which is to use our standard ggplot 
package.

And if we want to use ggplot, we need to get this matrix into a tidy format.

As a reminder, a tidy format is one where each row is an observation,
and each column is a variable.

Currently this matrix isn't in tidy format. 
...
```{r pivot-wide-to-long, echo=FALSE,  fig.align = "center", fig.height = 6.5,}
knitr::include_graphics(here("data-vis","figs","wide_to_long.png"))
...
```{r relational-data-cor-mat-2A, eval=FALSE}
cor_mat %>%
  as.data.frame()
```
...
```{r relational-data-cor-mat-2B, echo=FALSE}
cor_mat %>%
  as.data.frame() %>%
  kable() %>%
  kable_styling(font_size = 18) %>%
...
```{r relational-data-cor-mat-2C-wrong, eval=FALSE}
cor_mat %>%
  as.data.frame() %>%
  pivot_longer(cols = everything(), names_to = "variable", values_to = "r") %>%
```

<br>

```{r relational-data-cor-mat-2D-wrong, echo=FALSE}
cor_mat %>%
  as.data.frame() %>%
  pivot_longer(cols = everything(), names_to = "variable", values_to = "r") %>%
  kable() %>%
...
```{r relational-data-cor-mat-2C, eval=FALSE}
cor_mat %>%
  as.data.frame() %>%
  mutate(var1 = row.names(cor_mat))
```
...
```{r relational-data-cor-mat-2D, echo=FALSE}
cor_mat %>%
  as.data.frame() %>%
  mutate(var1 = row.names(cor_mat)) %>%
  kable() %>%
...
```{r relational-data-cor-mat-2E, eval=FALSE}
cor_mat %>%
  as.data.frame() %>%
  mutate(var1 = row.names(cor_mat)) %>%
  pivot_longer(cols = -var1, names_to = "var2", values_to = "r")
...
```{r relational-data-cor-mat-2F, echo=FALSE}
cor_mat %>%
  as.data.frame() %>%
  mutate(var1 = row.names(cor_mat)) %>%
  pivot_longer(cols = -var1, names_to = "var2", values_to = "r") %>%
...
ggplot(cor_tidy, aes(x = var1, y = var2)) +
...
```{r relational-data-cor-mat-4, echo=FALSE, fig.align="center", fig.height=5.2, fig.width=7.5}
```
