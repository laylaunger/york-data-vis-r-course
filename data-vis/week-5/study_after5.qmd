...
![](images/do_in_R.png) Make a scatterplot of `training_hours` (x-axis) versus `performance_index` (y-axis).  
...
![](images/do_in_R.png) Plot `training_hours` vs `wellbeing_index`, colouring points by `participant_group`.  Add group-specific trend lines.
...
![](images/do_in_R.png) Re-make the plot of `social_hours` vs `performance_index`, but facet by `job_role`.  Try `scales = "free_x"` inside `facet_wrap()`.
...
Reflect: Which variables are most strongly related to `performance_index`?
