...
```{r facet-grid-labels1, eval=FALSE}
p_scatter +
  facet_grid(sex~diagnosis, 
             labeller = labeller(sex = c(Female = "Sex: Female", Male = "Sex: Male")) ) + # label rows
  stat_smooth(aes(color = diagnosis), method = "lm") + 
  theme(legend.position = "none",
        strip.background = element_blank())
```
...
```{r facet-grid-labels1, echo=FALSE, fig.align="center", fig.height=5.25}
```
...
```{r facet-grid-labels2, eval=FALSE}
p_scatter +
  facet_grid(sex~diagnosis, 
...
```{r facet-grid-labels2, echo=FALSE, fig.align="center", fig.height=5.25}
```
