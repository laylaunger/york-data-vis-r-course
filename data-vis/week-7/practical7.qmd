...
In this practical you’ll start from basic plots and polish them using the strategies from **Seminar 7**:
...
We’ll use **two** CSVs from previous weeks. Save these in your project:
...
We’ll start with a simple scatterplot of **sleep vs wellbeing**.
...
- Use `labs()` to add a title and clearly label the axes (e.g., “Sleep (avg hours/night)”, “Wellbeing Index (0–100)”)
...
ggplot(wellbeing, aes(x = sleep_hours, y = wellbeing_index)) +
  ...
  labs(x = "Sleep (avg hours/night)", y = "Wellbeing Index (0–100)") +
  theme_bw() +
...
![](images/do_in_R.png) Create a tibble that computes each group’s **center** using `group_by` and `summarise`. Store the tibble, e.g., as an object called `centers`.
...
![](images/do_in_R.png) Look at the documentation for the labeller function using `?labeller` to see the many ways to customise `facet_wrap` and `facet_grid` labels
